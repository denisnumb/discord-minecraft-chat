        ...
        if (receivedParts.get(packet.imageId()).size() == packet.totalParts()) {
            ...
            executeServerCommand(prepareTellRawCommand(new ArrayList<>(){{
                ...
                add(new TellRawComponent(getTranslate(SCREENSHOT, "Screenshot"))
                        ...
                        .addClickEvent(new TellRawComponentEvent("open_url", message.getAttachments().getFirst().getUrl()))
                        .addHoverEvent(new TellRawComponentEvent("show_text", message.getAttachments().getFirst().getUrl()))
                );
            }}));
        }
    }
